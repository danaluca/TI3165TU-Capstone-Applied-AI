...
class Model:
    ...
    @staticmethod
    def data_reading(re: float = 40.0, nx: int = 24, nu: int = 2, shuf: bool = True, filename=None) -> np.array:
        """
    ...
    def preprocess(u_all: np.array or None = None, re: float = 40.0, nx: int = 24, nu: int = 2,
                   split: bool = True, norm: bool = True, filename=None) -> np.array or tuple[np.array]:
        """
        ...
        if u_all is None:
            u_all = Model.data_reading(re, nx, nu, filename=filename)
