...
class AE(Model):
    ...
    - Model hyperparameters: dimensions, sampler_dim, activation_function, l_rate, epochs, batch, early_stopping, pooling, loss
    - Data: u_train, u_val, u_test
    - Keras models: autoencoder, encoder, decoder
    - Other: ae_type, re, nx, nu, y_pred
    ...
    # Initialize model
    def __init__(self, dimensions: list[int] = [240, 120, 60, 30], sampler_dim=8, l_rate: float = 0.0005, epochs: int = 200,
        ...
        :sampler_dim: int, the layer size of the sampler when a VAE model is used
        ...
        :param ae_type: str, sets the model to be a conventional, hierarchical or variational autoencoder
        """
        ...
        if self.ae_type == 'conventional':
            self.network()
        elif self.ae_type == 'hierarchical':
            self.h_network()
        elif self.ae_type == 'variational':
            self.v_network()
        else:
            raise ValueError(f"AE type '{self.ae_type}' is not one of 'conventional', 'hierarchical' or 'variational'. ")
    ...
    def network(self) -> None:
        ...
        print(x.shape)
        x = self.pooling_function((3, 3), padding='same')(x)
        print(x.shape)
    ...
    def v_network(self) -> None:
        """
        This function defines the architecture of the variational autoencoder. Every layer of the NN presents a convolution
        and a pooling. There are 8 layers in total, 4 for the encoder and 4 for the decoder.
            Conv2D inputs:
            - number of features to be extracted from the frame
            - (m, n) defines the size of the filter, x_size_filter = x_size_frame / m, y_size_filter = y_size_frame / n
    ...
    def training(self) -> None:
        ...
        if self.ae_type == 'conventional' or self.ae_type == 'hierarchical':
    ...
    def create_trained(type: int = 1) -> object:
        ...
        match type:
            case 1:
                model = AE(ae_type='hierarchical', loss=custom_loss_function)
                # Autoencoder
